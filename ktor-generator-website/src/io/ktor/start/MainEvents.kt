...
fun registerKeyboardUsability() {
    document.addEventListener("keydown", { be ->
        ...
        for (filterId in listOf("dependency-filter-client", "dependency-filter-server")) {
            val dependencyFilter = document.getElementById(filterId)

            if ((active?.tagName != "INPUT") && (active?.tagName != "TEXTAREA") && (e.key in listOf("f", "s", "c"))) {
                if (
                    ((e.key === "f" || e.key === "s") && filterId.contains("server")) || (e.key === "c" && filterId.contains("client"))
                ) {
