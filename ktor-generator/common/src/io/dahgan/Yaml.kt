...
object Yaml {
    ...
    private fun load(tokens: Sequence<YamlParser.Token>): List<Any> {
...
@Suppress("MemberVisibilityCanBePrivate", "FunctionName", "ObjectPropertyName", "ObjectPropertyName",
    "UNUSED_PARAMETER"
)
...
// 7.5 Flow Nodes
    ...
    fun l_block_mapping(n: Int) = detect_collection_indentation(n).snd(
        "m", wrapTokens(Code.BeginMapping, Code.EndMapping,
            { state ->
                val m = state.yields["m"] as Int
    ...
    fun ns_l_block_map_entry(n: Int) = wrapTokens(
        Code.BeginPair, Code.EndPair, c_l_block_map_explicit_entry(n) or
...
open class YamlParser {
     ...
     */
    fun commit(decision: String): Parser = { state -> Reply(Result.Completed(""), emptySequence(), decision, state) }
    ...
    companion object : YamlParser() {
         ...
         */
        fun escape(text: IntArray, separator: String = ", "): String = text.map(YamlParser.Companion::escape).joinToString(separator)
